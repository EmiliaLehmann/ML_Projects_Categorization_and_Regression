...
def save_results_to_files(param_name, val, avg_train, avg_test):
    ...
    # 1. Zapis do CSV (do tabel i wykresów w Excelu)
    ...
    # 2. Zapis do TXT (czytelny raport tekstowy)
...
analysis_config = {
    "num_layers": [1, 2, 3, 4],            # NOWE
    "activation": ['tanh', 'relu', 'sigmoid', 'elu'], # NOWE
...
for param_name, values in analysis_config.items():
    ...
    for val in values:
        ...
        for r in range(n_repeats):
            ...
            # --- TEST ---
            # Pętla startuje od wyznaczonego punktu tr_size do samego końca danych
            test_errors = []
        ...
        # Zapis do plików (val będzie ułamkiem, np. 0.1, 0.2...)
...
print("\nWSZYSTKIE EKSPERYMENTY ZAKOŃCZONE. Wyniki w raport_koncowy.txt oraz wyniki_analizy_rnn.csv")